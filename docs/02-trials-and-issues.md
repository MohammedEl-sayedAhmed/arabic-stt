...
| 31 | Your meetings: a dataset agent built a private set from your saved meeting transcripts and the meeting platform's recordings (read-only): 6 meetings, 9 h 51 min, verified verbatim ElevenLabs references with corrected speakers | the platform's API hit its daily limit (cached snapshots used); two variants of one meeting turned out to be **translations** (English Arabized) | Kept outside the repo; translated variants quarantined; audio cut exactly as sent to the meeting-minutes app |
