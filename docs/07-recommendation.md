...
Don't use R2T2 for this. It runs on a CPU, so the handoff was too pessimistic on that point, but it
is one of the two least accurate models tested on Egyptian Arabic–English: 47.1% WER on Perle with
the whole clip, against 15.7% for Cohere measured the same way and 13.4% with the current pipeline.
It is no better than its base model, and it writes English terms in Arabic letters. Its strength,
low-latency live streaming, doesn't matter for recordings.
...
reference here: it has a free tier, costs $0.22/h after it, and has a setting to stop it training on
your audio. Speechmatics (`ar_en`, $100 of free credit, no training on your audio) is the other
...
- Licences: Cohere (Apache-2.0) and the whisper-medium fine-tune (MIT) allow commercial use, but
  check the fine-tune's training-data licence (its card mentions GPL). Audar's licence allows
  research and evaluation only, and R2T2's is a custom NetEase licence under PRC law. The MMS
  forced-aligner model behind `--align` is non-commercial (CC-BY-NC-4.0).
