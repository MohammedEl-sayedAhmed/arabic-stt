...
def write_tables(summary, path):
    out = ["# Result tables (generated by `bench/score.py --tables`)", "",
           ...
           "timestamps, gap-fill).", ""]
    summary = current_rows(summary)
    for test_set in ("perle", "arzen"):
        for audio in ("wav16k", "phone", "g711"):
            rows = sorted([s for s in summary if s["set"] == test_set and s["audio"] == audio], key=lambda s: s["wer"])
            if not rows:
                continue
            out += [f"## {test_set} — {AUDIO_NAMES.get(audio, audio)}", "",
                    "| Model | Setting | WER (95% CI) | CER | Arabic-word errors | English-word errors "
                    "| English kept | Speed |", "|---|---|---|---|---|---|---|---|"]
            for s in rows:
                lo, hi = s["wer_ci"]
                label = s["label"] + (" *(whole clip)*" if s["config"].endswith("-clip") else "")
                out.append(f"| {MODEL_NAMES.get(s['model'], s['model'])} | {label} | {s['wer']:.1%} "
                           f"({lo:.0%}–{hi:.0%}) | {s['cer']:.1%} | {s['ar_err']:.1%} | {s['en_err']:.0%} "
                           f"| {s['en_kept']:.0%} | {s['rtf']:.2f} |")
            out.append("")
